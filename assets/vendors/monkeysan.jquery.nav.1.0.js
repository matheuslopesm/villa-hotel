	...
	DesktopState.prototype.init = function(){
		...
		_self.element.on('click.MonkeysanNavDesktop', 'a', function(event){
			...
			if(target != '_blank') {
				...
				setTimeout(function(){
					var $preloader = $body.find('.' + _self.config.cssPrefix + _self.config.classes.preloader);

					$body.removeClass(_self.config.cssPrefix + _self.config.classes.bodyMovingToAnotherPage);

					if($preloader.length) {
						$preloader.removeClass(_self.config.cssPrefix + _self.config.classes.preloaderAppearingClass);
					}
